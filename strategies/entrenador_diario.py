...
def obtener_datos_diarios(simbolo, anos=5):
    print(f"[{simbolo}] Descargando 5 años de datos diarios...")
...
def entrenar_modelo_diario(simbolo):
    print(f"\n{'='*40}\nIniciando pipeline DIARIO para {simbolo}\n{'='*40}")
    df = obtener_datos_diarios(simbolo)
    ...
    X, y = crear_dataset(data_x_escalada, data_y, VENTANA_DIAS)
    ...
    modelo.save(f'models/lstm_{simbolo}_1D.keras') # Guardado como 1D (Diario)
    joblib.dump(scaler, f'models/scaler_{simbolo}_1D.pkl')
    print(f"✅ [{simbolo}] Modelo Diario Guardado.\n")

if __name__ == "__main__":
    for activo in PORTAFOLIO:
        entrenar_modelo_diario(activo)
